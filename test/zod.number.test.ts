...
describe('NumberGenerator', () => {
    ...
    describe('invalid', () => {
        ...
        it('should generate numbers outside of min-max range', () => {
            const schema = z.number().min(10).max(20);
            const result = generator.invalid(schema);
            const tooSmall = result.find(r => typeof r.value === 'number' && r.value < 10);
            const tooBig = result.find(r => typeof r.value === 'number' && r.value > 20);
        ...
        it('should generate non-finite numbers for finite schema', () => {
            const schema = z.number().finite();
            const result = generator.invalid(schema);
            const infiniteCase = result.find(r => typeof r.value === 'number' && !Number.isFinite(r.value));
